...
class DIR(nn.Module):
    ...
    def do_prediction(self, x, factor):
        ...
        with torch.no_grad():
            ...
            ## first level
            t = torch.empty(x.shape[0], dtype=torch.int32 ).fill_(self.treatment_levels[0])
            t = t.view(-1,1)
            pred_y0, pred_log_t, pred_log_e, l_t, l_ty, l_e, l_ey, l_y = self.forward(x=x, t=t)
            
            probs_t = F.softmax(pred_log_t, dim=1)
             ## treatment_level must be zero based
            prob_val_t_base = probs_t[:, self.treatment_levels[0]]
            ...
            for i in range(1, len(self.treatment_levels)): 
                ...
                pred_y, pred_log_t, pred_log_e, l_t, l_ty, l_e, l_ey, l_y = self.forward(x=x, t=t)
                ...
                tempScore = pd.DataFrame((pred_y.numpy()-pred_y0.numpy() ) , columns =[factor + self.separation + str(self.treatment_levels[i])])
    ...
    def do_train(self, iterator, optimizer, epoch):
        ...
        for i, batch in enumerate(iterator):
            ...
            #loss = y_risk_loss + self.args.beta1*t_prob_loss + self.args.beta2*e_prob_loss
    ...
    def _compute_loss(self, batch):
        ...
        prob_val_e = 1 - prob_val_e
        
        prob_val_e[prob_val_e < 0.001] = 0.001
        
        
        sample_weight_t = 1/(prob_val_t*prob_val_e)
        
        y_risk_loss = helpers.risk_loss(pred_y, torch.tensor(e).long().squeeze(), y.squeeze(), torch.tensor(sample_weight_t))
